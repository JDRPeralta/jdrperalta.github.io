...
export default function App() {
  ...
  const filteredProducts = useMemo(() => {
    return PRODUCTS.filter((product) => {
      ...
      const matchesSearch =
        !searchQuery ||
        `${product.name} ${product.description} ${product.category}`.toLowerCase().includes(searchQuery.toLowerCase());
                ...
                {(section === "cart" || section === "orders") && (
                  <span
                    className={`text-xs font-bold px-1.5 py-0.5 rounded-full ${
                      section === "cart" ? "bg-blue-100 text-blue-700" : "bg-slate-100 text-slate-700"
                    }`}
                  >
                              ...
                              <div className="mt-4 flex justify-between items-center">
                                ...
                                <div className="flex items-center space-x-2">
                                  ...
                                  <button
                                    ...
                                    className="bg-blue-600 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded-xl transition-colors shadow-sm hover:shadow"
                                  >
                                    Agregar
                                  </button>
                                </div>
                        ...
                        {order.items.map((item, index) => {
                          ...
                          return (
                            <div key={index} className="text-slate-700">
                              • {item.quantity} × {product ? product.name : `Producto #${item.productId}`}
                            </div>
                          );
                        })}
                        <div className="text-xs text-slate-500 mt-2">Entrega: {order.customerAddress}</div>
                      </div>
            ...
            >
              ...
              <div className="p-5">
                ...
                <div className="mt-6 p-4 bg-blue-50 rounded-xl">
                  <div className="text-sm font-medium text-slate-700 mb-2">Resumen del pedido</div>
                  <div className="flex justify-between text-sm mb-1">
                    <span>Subtotal</span>
                    <span className="font-medium">S/ {cartSummary.subtotal.toFixed(2)}</span>
                  </div>
                  <div className="flex justify-between text-sm mb-1">
                    <span>Delivery</span>
                    <span className="font-medium">
                      {cartSummary.delivery === 0 ? "Gratis" : `S/ ${cartSummary.delivery.toFixed(2)}`}
                    </span>
                  </div>
